...
Личный репозиторий подготовки к сертификации **HashiCorp Certified: Terraform
Associate (004)**.

Все объективы разделов 1–8 закрыты. Заметки организованы так: **один `.md`
файл в `notes/` = один объектив экзамена**, с явными scope-notes, где темы
пересекаются, чтобы факты жили в одном месте и не расползались по нескольким
файлам.
...
.
├── README.md          # этот файл
├── CHEATSHEET.md       # сжатая выжимка для повторения в день экзамена
└── notes/              # подробный разбор, один файл = один объектив
    ├── 01a-what-is-iac.md
...
**Прогресс: 8 / 8 разделов, 37 / 37 объективов.**

## Как пользоваться

* **`CHEATSHEET.md`** — для быстрого повторения в день экзамена. Только
  факты, которые реально ловят на тесте: edge cases, дефолты, "X никогда не
  Y", различия похожих концепций. Сгруппирован по темам, а не по файлам —
  повторы между объективами убраны.
* **`notes/`** — для глубокого разбора. Каждый файл покрывает ровно один
  объектив и содержит `> Scope note`, если тема пересекается с другим
  объективом — это укажет, где искать полную механику, если что-то кажется
  урезанным.

Внутри каждого файла заметок: **Core Concept** → детали объектива → **Exam
Quick Facts** в конце — сжатый список ловушек именно этого файла (в
`CHEATSHEET.md` эти списки сведены воедино и очищены от повторов).
